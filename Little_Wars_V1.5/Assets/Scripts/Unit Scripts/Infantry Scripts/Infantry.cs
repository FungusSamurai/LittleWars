...
/// <summary>
/// Information and Behavior Script for the Infantry unit, inherits from the Unit abstract class.
/// </summary>
public class Infantry : Unit
{
    public static int PointValue = 1;
    public static int MaxMoveSpeed = 3;
    ...
    void OnCollisionEnter(Collision col)
    {
        if (!dying)
        {
            ...
            else if (col.gameObject.tag == "Cannonball")
            {
                ...
                sfx.transform.position += transform.right * Random.Range(-1, 2); //and slightly to the side
    ...
    IEnumerator Falling()
    {
        ...
        while(waitForIt > 0)
        {
            if (rigid.velocity.sqrMagnitude > 1f)
                waitForIt = 3; //If the object is still moving we keep waiting
    ...
    public override bool Moved { get { return moved; } set { moved = value; } } // if true, moved means the unit has been moved this turn.
}
